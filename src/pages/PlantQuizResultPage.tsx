...
function PlantQuizResultPage() {
  ...
  const cloud_url =
    ...
    "https://res.cloudinary.com/dvdr5bwc7/image/upload/c_fill,f_auto,q_auto";
  ...
  const { vote, submitFeedback } = useFeedback({
    quizSessionId,
    plantId: recommendations[0]?.id,
  });
...
console.log(result.data);
        setQuizSessionId(result.data.session_id);
  ...
  return (
    <section>
      ...
      <section>
        <div className="flex items-center justify-center mb-12 max-w-md mx-auto">
          <div className="flex items-center w-full m-4">
            <div
              className={`flex items-center justify-center w-10 h-10 rounded-full ${step === "questionnaire" || step === "analyzing" || step === "results" ? "bg-amber-600 text-muted" : "bg-stone-200 text-stone-500"}`}
            >
              <OrangeCheckIcon />
            </div>
            <div
              className={`flex-1 h-1 mx-2 ${step === "analyzing" || step === "results" ? "bg-amber-600" : "bg-amber-600"}`}
            />
            <div
              className={`flex items-center justify-center w-10 h-10 rounded-full ${step === "analyzing" || step === "results" ? "bg-amber-600 text-white" : "bg-stone-200 text-stone-500"}`}
            >
              <OrangeCheckIcon />
            </div>
          </div>
        ...
        <article className="bg-text-inverse h-fit m-4 rounded-2xl overflow-hidden shadow-lg pb-6">
          <div className="bg-success-500  shadow-lg p-4 md:p-12 flex justify-between">
            <div className="flex flex-col gap-0">
              ...
              <p className="text-[.7rem] text-text-inverse">
                Recommended for your space
              </p>
            </div>
            <div className="w-fit p-2  bg-text-inverse/40 rounded-3xl flex  items-center">
              <p className="text-xs text-center text-text-inverse">
                {firstRecom.scoreMatch * 100}% Match
              </p>
            </div>
          </div>
          ...
          >
            <div className="relative overflow-hidden h-[15rem] m-4">
              ...
              <div className="absolute inset-0 z-20 flex gap-0 items-center justify-center  opacity-0 group-hover:opacity-100 transition-all duration-500 transform scale-90 group-hover:scale-100">
                <span
                  className="flex  bg-text-inverse/30 py-2 px-3 text-text-inverse font-bold rounded-full text-xs uppercase tracking-[0.2em] shadow-2xl m-0       
            "
                >
                  Explore
                  <ArrowRight className="w-4 h-4 text-text-inverse transition-tranform duration-300 ease-out transform group-hover:translate-x-1.5" />{" "}
                </span>{" "}
              </div>
            </div>
          </Link>
          ...
          <p className="italic text-xs mx-4 text-text-muted">
            {firstRecom.scientific_name}
          </p>
          ...
          <div className="flex gap-2 items-center">
            <div className="w-fit p-2 px-4 m-4 bg-success-300/40 rounded-3xl flex  items-center">
              <span className="text-success-500 text-xs">
                {firstRecom.plantinglevel}
              </span>
            </div>
            <p className="text-center text-amber-600">
              $
              {(
                firstRecom.sizes[0].original_price *
                ((100 - firstRecom.sizes[0].discount_percentage) * 0.01)
              ).toFixed(2)}
            </p>
          </div>

          <div className="m-4">
            <h2 className="text-xxs my-4">
        ...
        <article className="bg-text-inverse h-fit m-4 rounded-2xl overflow-hidden shadow-lg pb-6">
          ...
          {recommendations.slice(1, 4).map((recoms, id) => (
            <Link
              to={`/products/${recoms.slug}`}
              className="cursor-pointer hover:shadow-lg "
            >
              <SecondaryRecommendedCard
                secondRecom={recoms}
      ...
      {quizSessionId && <ChatbotWindow quizSessionId={quizSessionId} userId={123} />}
      <Footer />
    </section>
  );
}
